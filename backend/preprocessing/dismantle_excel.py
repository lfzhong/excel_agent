...
def drop_and_merge_excel(excel_info, merged_info):
    ...
    system_prompt = """你是一个Excel数据处理专家，专门负责分析Excel表格结构并确定需要删除的说明性文本行和表头行。

                    你的任务是：
                    1. 分析提供的Excel数据，找出哪些行是说明性文本（标题、注释、说明等）需要删除
                    2. 确定哪些行是真正的表头行（通常包含列名）
                    3. 返回JSON格式的结果

                    输入格式：
                    - excel_info: 包含每个sheet的前几行数据
                    - merged_info: 包含合并单元格的信息

                    输出格式：
                    [
                    {
                        "sheet_name": {
                        "labels": [行号列表],  // 需要删除的行号（从1开始）,如果没有需要删除的行，则返回[]
                        "header": [行号列表]   // 表头行号（从1开始）,有些sheet可能多表头，则返回多个表头行号
                        }
                    }
                    ]
                    ...
                    注意：
                    - 行号从1开始计数
                    - labels中的行会被删除，所以header行号需要考虑删除后的位置
                    - 只返回JSON，不要其他文本"""

    prompt = f"""请分析以下Excel数据：
...
请确定每个sheet中：
1. 哪些行是说明性文本需要删除（labels）
2. 哪些行是表头行（header）

返回JSON格式的结果。"""

    client = LLMClient()
    response = client.chat(prompt, system=system_prompt)
                ...
                for sheet in lable_info_json:
                    for sheet_name, config in sheet.items():
                        try:
                            ...
                            logger.error(f"处理表单 {sheet_name} 时出错: {e}")
                        finally:
                            if os.path.exists(drop_file):
                                os.remove(drop_file)

            if sheets_processed == 0:
                logger.error("没有成功处理任何表单，删除输出文件")
